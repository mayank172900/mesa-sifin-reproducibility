...
\hypersetup{
  ...
  citecolor=blue,
  urlcolor=blue,
  pdftitle={Mathematical Appendix for Structural Hawkes Ambiguity in Near-Critical Market Making},
  pdfauthor={Mayank Sharma and Rohit Mourya},
  pdfsubject={Mathematical appendix for spectral and control-theory details},
  pdfkeywords={Hawkes processes, Perron theory, robust market making, structural ambiguity, HJBI, Lyapunov comparison}
}
\urlstyle{same}
...
\title{Mathematical Appendix for Structural Hawkes Ambiguity in Near-Critical Market Making}
\author{}
\date{}

\begin{document}
\begin{center}
{\LARGE\bfseries Mathematical Appendix for Structural Hawkes Ambiguity in Near-Critical Market Making\par}
\end{center}
\vspace{0.75em}

\section{Purpose}

This companion appendix records the proof details that would slow the main
paper's narrative. MESA means market-making excitation-structure ambiguity. The
first sections cover the scalar amplification result, a Perron-visible multitype
bridge, the finite-state Bellman layer used in the dynamic program, and a
compact HJBI verification result for the truncated multitype game. The later
sections spell out the extra conditions needed beyond truncation: robust
subcriticality, weighted comparison, and local sensitivity of interior quote
optimizers. The scope is deliberately narrow: the appendix covers only the
model class used in the main paper and makes the assumptions needed for
extensions explicit.

\section{Notation Used Below}
...
Unless a theorem says otherwise, asymptotic constants are taken uniformly over
the compact regularity classes stated there. The same letters may denote a
...
\qquad C>0,
\]
where $a=2$ matches the normalized-throughput variance convention used for the
square-law statement in the main paper. If stationary throughput is modeled
separately in the risk term, $a$ can be larger; the proofs keep that dependence
explicit.
...
below gives one sufficient route: a uniformly negative quote-space second
derivative together with a smooth active worst-case selector for the robust
Hamiltonian.
...
We distinguish two ambiguity sets. This is a small modeling choice with a large
near-critical consequence, because the normalization changes the singular
exponent.
...
\]
If the local quote-sensitivity assumption holds uniformly near the reference
regime, then, for small $\eps$,
\[
\psi_\eps(\rho)
=KaC\,\eps(1-\rho)^{-a}+O(\eps^2(1-\rho)^{-a})+o(\eps(1-\rho)^{-a}).
\]
Within this reduced proxy, in the normalized-throughput convention $a=2$, the
premium has the square law
...
$(1-\eps)^{-a}=1+a\eps+O(\eps^2)$ gives the small-$\eps$ expression. The
...
\]
If the local quote-sensitivity assumption holds uniformly near the reference
regime, then, within this reduced proxy, the normalized-throughput convention
$a=2$ gives the premium order
$\Theta(\eps(1-\rho)^{-3})$.
\end{theorem}
...
$\rho<1$, normalized left/right Perron vectors $\ell^\top r=1$, spectral
gap at least $g_0>0$, uniformly bounded Perron eigenprojection, and uniformly
bounded reduced resolvent on the non-Perron spectral subspace. Then, for
sufficiently small perturbations $E$,
\[
\rho(\Gamma+E)-\rho(\Gamma)=\ell^\top E r+O(\|E\|_F^2/g_0).
\]
Among unconstrained Frobenius-unit directions, the first-order maximizer is
$E^\star = \ell r^\top/\|\ell r^\top\|_F$. Feasibility of a scaled perturbation
inside a particular ambiguity set must be checked separately.
\end{proposition}
...
\begin{remark}
This proposition explains the Perron-aligned adversary used in the experiments
when the leading eigenvalue is simple and well separated. It also explains why
near-degenerate spectra are a real scope condition rather than a nuisance
parameter: the lower bound can weaken when Perron regularity fails.
\end{remark}
...
\]
and uniformly bounded Perron eigenprojection and reduced-resolvent
conditioning. Let $r,\ell>0$ be normalized by $\ell^\top r=1$. For fixed
$\mu,w\in\R_+^d$, define
\[
...
\]
and the perturbation path remains in the same compact Perron-regular class with
Perron visibility bounded away from zero and no first-order cancellation,
then
\[
...
\qquad \eps=o(1-\rho),
\]
and the same Perron-regularity and visibility conditions remain in force,
then
\[
...
This section records the finite-state control layer used by the implemented
experiment package. It gives the robust dynamic-programming equation for the
finite inventory, finite-regime approximation used numerically. By itself, this
...
maximum and minimum are optimal. The corresponding discrete-time statement
holds for a stochastic one-step kernel; for the explicit Euler kernel this
requires $\Delta t$ small enough that $I+\Delta t L^{u,\Gamma}$ has nonnegative
entries and row sums one.
\end{theorem}
...
\begin{remark}
This theorem supports the finite-scenario Bellman layer used in the code. If
...
differentiability of the untruncated optimizer with respect to $\Gamma$. The next
section gives the compact continuous-intensity bridge.
\end{remark}

\section{Compact Continuous-Intensity HJBI Bridge}

This section packages the compact-domain verification step, conditional on the
stated compact-PDMP dynamic-programming and comparison hypotheses. The argument
uses standard PDMP, stochastic-control, and viscosity-solution machinery
\cite{Davis1993,FlemingSoner2006,CrandallIshiiLions1992}. The next two sections
remove the projection through a Lyapunov/tightness argument and a weighted
comparison theorem, and the consolidated theorem below records the
resulting untruncated statement.
...
\begin{theorem}[Conditional compact continuous-intensity Hawkes HJBI verification]
...
\begin{remark}
This theorem records the compact truncated bridge conditional on the standard
...
pointwise $\rho(\Gamma)<1$. This is the tradeoff required by a uniform
untruncated theorem.
\end{remark}
...
This section states a sufficient comparison condition for the unbounded-domain
HJBI bridge, following the standard viscosity comparison strategy with a
weighted barrier and a doubling-of-variables argument
\cite{CrandallIshiiLions1992,FlemingSoner2006}. It is deliberately explicit:
without a weighted
...
\begin{enumerate}
\item The ambiguity set $\mathcal A$ is compact, and the control set is either
a fixed compact set or a compact correspondence $U(q,\lambda)$ continuous in
Hausdorff distance on compact $\lambda$-sets; measurable $\eta$-selectors
exist for the local Hamiltonian.
...
\begin{theorem}[Sufficient weighted-comparison bridge for the untruncated Hawkes HJBI]
...
\]
for every $(t,q,\lambda)$. Consequently, any two candidates in the same
weighted-boundary subclass are ordered by comparison. Any compact-truncation
limit satisfying this boundary condition is therefore uniquely identified, and
...
\begin{remark}
This theorem records the untruncated HJBI comparison bridge for the model class
...
assumptions. The theorem below gives a single citation point for the sufficient
conditions assembled above.

\begin{theorem}[Consolidated conditional multitype Hawkes control bridge]
...
optimizer assumption, with inactive quote constraints, a unique $C^1$ active
worst-case structural selector, and a $C^2$ reduced Hamiltonian with continuous
...
\begin{proof}
Items 1 and 2 follow from the Lyapunov stability theorem: the common
...
localization assumptions. Item 6 is the interior optimizer
...
$\Theta(\eps_{\mathrm{rel}}(1-\rho)^{-2})$. Under local quote sensitivity, if
the finite ambiguity set includes this Perron-aligned scenario and that scenario
is selected as worst case, then the half-spread has the same square-law order.
\end{theorem}
...
\]
In particular, the leading singular order is
$\Theta(\eps(1-\rho)^{-3})$. Under local quote sensitivity, if the finite
ambiguity set includes this Perron-aligned scenario and that scenario is
selected as worst case, then the half-spread has the same derivative-law order.
\end{theorem}
...
choosing $t=O(\eps)$ gives the absolute theorem. This matches the
Perron-aligned direction used in the matrix-resolvent experiments, up to the
feasibility and normalization choices imposed there.
\end{remark}

\section{Conclusion}

The square-law statement should be read with its normalization in view. When
the ambiguity radius is measured relative to the remaining critical slack, or
when the risk quantity has already been normalized, the reduced premium has the
square-law order. When $\eps$ is an absolute branching-matrix radius, the robust
premium inherits one additional derivative of the critical singularity. The same
distinction holds in the bivariate Perron-visible setting. In that sense, this
appendix puts the MESA claim into precise, testable scalar and bivariate theorem
statements.
