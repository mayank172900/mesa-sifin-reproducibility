...
reproducibility-package organization. The author reviewed the outputs and
remains responsible for all mathematical claims, proofs, citations, code, data
...
I first test the critical-scaling predictions in controlled ablations, then
...
To separate the exponent from the particular stress design, I use a targeted
...
positive paired mean-wealth difference. I do not interpret the
...
sensitive to bin width, so I repeat the focused policy comparison at
...
I next compare the fast binned Hawkes recursion with event times generated by
...
Finally, I run a reduced event-driven queue backtest on Ogata Hawkes event
...
I then let away quotes interact with displayed depth by replaying the uniform
...
As a depth supplement, I also replay the deepest public LOBSTER files available
...
I also run a priority-assumption sensitivity grid for the nominal policy. The
...
Finally, I check whether the public messages support the queue analysis near
the touch. I reconstruct the observable level-10 book from messages and compare
...
I also run a quote-map sensitivity diagnostic linking the local optimizer
...
As a check before interpreting real-data Hawkes fits, I validate the corrected
...
I also validate the fixed-beta marked multivariate estimator on
...
To avoid relying on one crypto day, I repeat the Binance check on
...
Finally, to align the empirical object with the bid/ask control model, I fit a
...
I then condition the six-mark time-rescaling residuals on observable L1 state
